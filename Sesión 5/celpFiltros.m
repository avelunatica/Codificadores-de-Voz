function [sh,B,G,AK,Tv,indv, bitsPorMuestra] = celpFiltros(s,Ltrama,Lsubtrama,p)
...
z_w_iniciales=[];
z__inv_W_iniciales=[];
z_fmod_iniciales=[]; % nuevo filtro
z_inv_fmod_iniciales=[];

% Entrada nula para el nuevo filtro
...
for i = 1:floor(length(s)/Lframe) % Para cada trama
    ...
    for n = 1:4 % Para cada subtrama
        ...
        [u_n z_w_finales] = filter(ak,akPond,subtrama_actual, z_w_iniciales);
        z_w_iniciales = z_w_finales;
        
        [y3 z_fmod_finales] = filter(1,akPond, EntradaNula, z_fmod_iniciales);
        z_fmod_finales = z_fmod_finales';
        ...
        for t = 1 : length(libraria_adaptativa) - Lsubtrama + 1 
            ...
            [y20, z_adap_final] = filter(1,akPond,d20, []); %LPC síntese
            b=(u3*y20.')/(y20*y20'+ eps);  %tendo en conta n=1:4 
            y2=y20*b;
        ...
        zFilAdap = matriz_z_libraria_adaptativa(Indice_t,:); 
        ...
        for j = 1:M % Seguindo as diapos debería ser unha l, pero non diferencio 'l' de '1'
            ...
            [y10, z_est_final_c] = filter(1,akPond,v(j,:), []); %LPC síntese
         ...
         zFilEst = matriz_z_libraria_estocastica(Indice_indv,:);
         ...
         u3_reconstruida = Y2(Indice_t,:) + Y1(Indice_indv,:);
         
         % ESTO ES LO QUE ESTÁ MAL :
         % -----------------------------------------------------------------------------------
         % Aquí se tendría que reconstruir la subtrama original a partir de u3_reconstruida        
         u_reconstruida = u3_reconstruida + y3;
         % Ahora para reconstruir la señal de voz será necesario pasar y2(t)+y1(l) por 1/W(Z)
         [shi z_inv_W_finales] = filter(akPond,ak,u_reconstruida, z__inv_W_iniciales);
         z__inv_W_iniciales = z_inv_W_finales; % actualizo condiciones del filtro para la sig. iteración
         sh =[sh shi];
         % ___________________________________________________________________________________
         
         
         % Actualizo las condiciones del nuevo filtro
         z_fmod_iniciales = z_fmod_finales + zFilEst + zFilAdap;
