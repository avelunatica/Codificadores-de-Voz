function [sh,B,G,AK,Tv,indv, bitsPorMuestra] = celp(s,Ltrama,Lsubtrama,p)
...
for i = 1:floor(length(s)/Lframe) % Para cada trama
    ...
    for n = 1:4 % Para cada subtrama
        ...
        for t = 1 : length(libraria_adaptativa) - Lsubtrama + 1 
            ...
            [y20, z_adap_final] = filter(1,akPond,d20, z_adap_inicial); %LPC síntese
            b=(u_n*y20.')/(y20*y20'+ eps);  %tendo en conta n=1:4 
            y2=y20*b;
            u0 = u_n - y2; % en la cuatro era subtrama actual (sin filtrar) - y2
            Et=sum(u0.*u0);
        ...
        for j = 1:M % Seguindo as diapos debería ser unha l, pero non diferencio 'l' de '1'
            ...
            [y10, z_est_final_c] = filter(1,akPond,v(j,:), z_est_inicial); %LPC síntese
         ...
         [shiFiltrada z_inv_W_finales] = filter(akPond,ak,shi, z__inv_W_iniciales);
